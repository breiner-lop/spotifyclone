...
const NavTop = () => {
    ...
    const [scrollDown,setScrollDown]=useState(false)
            ...
            useEffect(() => {
              document.getElementById("scrollcontainer")?.addEventListener('scroll',trackScrolling);
              return () => {
                document.getElementById("scrollcontainer")?.removeEventListener('scroll',trackScrolling);
              }
            },[router])
    return (
        <div className={`flex justify-between absolute z-50  py-4 pr-8 pl-8 w-full ${scrollDown?"bg-black":"bg-transparent"}`}>
        <div className='flex'>
